...
(function () {
  ...
  /* flow-dot(이동 표시점) 이동에 쓰이는, 인체 도해 SVG 좌표(대략적인 장기 중심) */
  const ORGAN_COORDS = {
    brain: { x: 160, y: 58 },
    skin: { x: 160, y: 70 },
    liver: { x: 118, y: 186 },
    stomach: { x: 204, y: 196 },
    intestine: { x: 150, y: 262 },
    kidney: { x: 160, y: 216 },
  };
  ...
  let explanationLevel = "simple"; // "simple" | "detailed" — 실제 기전 설명 난이도
  ...
  const el = {
    ...
    fOrgan: $("#f-organ"),
    stepFieldStomach: $('[data-organ-step="stomach"]'),
    stepFieldIntestine: $('[data-organ-step="intestine"]'),
    stepFieldLiver: $('[data-organ-step="liver"]'),
    stepFieldKidney: $('[data-organ-step="kidney"]'),
    stepFieldSkin: $('[data-organ-step="skin"]'),
    ...
    btnLevelSimple: $("#btn-level-simple"),
    btnLevelDetailed: $("#btn-level-detailed"),
    flowDot: $("#flow-dot"),
  };
  ...
  function getRealText(device) {
    if (explanationLevel === "simple" && device.realMechanismSimple && device.realMechanismSimple.trim()) {
      return device.realMechanismSimple;
    }
    return device.realMechanism || "";
  }

  function renderCompare(device) {
    el.textAd.textContent = device.adClaim || "";
    el.textReal.textContent = getRealText(device);
    el.textWarn.textContent = device.sideEffects || "아직 등록된 부작용 정보가 없습니다.";
  }

  function setExplanationLevel(level) {
    explanationLevel = level;
    el.btnLevelSimple.classList.toggle("active", level === "simple");
    el.btnLevelDetailed.classList.toggle("active", level === "detailed");
    const device = DB.get(currentDeviceId);
    if (device) el.textReal.textContent = getRealText(device);
  }
   ...
   */
  function stepCaption(device, key, fallback) {
    const custom = device.stepCaptions && device.stepCaptions[key];
  ...
  function buildRealSteps(device) {
    ...
    if (target === "skin") {
      return [
        { organ: "skin", caption: stepCaption(device, "skinEntry", "피부 표면 각질층에 발라져 스며들기 시작합니다.") },
        { organ: "skin", caption: getRealText(device) },
      ];
    }

    if (device.deliveryRoute === "sensory") {
      // 향·소리·촉감처럼 소화관을 거치지 않고 감각신경을 통해 곧장 뇌로 전달되는 경로
      return [
        { organ: target, caption: stepCaption(device, "sensoryEntry", "감각 신경을 통해 신호가 뇌로 곧장 전달됩니다.") },
        { organ: target, caption: getRealText(device) },
      ];
    }
    ...
    PIPELINE_ORDER.forEach((organ) => {
      if (organ === target) {
        steps.push({ organ: organ, caption: getRealText(device) });
        targetInserted = true;
      } else {
        steps.push({ organ: organ, caption: stepCaption(device, organ, PIPELINE_CAPTION[organ]) });
      }
    });

    if (!targetInserted) {
      // 표적 기관이 파이프라인 밖(예: 뇌)이면, 신장(배출) 직전에 끼워 넣습니다.
      steps.splice(steps.length - 1, 0, { organ: target, caption: getRealText(device) });
    }
  ...
  /* 이동 표시점(flow-dot): 재생 중 몸속을 실제로 "이동"하는 것처럼 보여주는 점 */
  function updateFlowDot(organId, animate) {
    const coords = ORGAN_COORDS[organId];
    if (!coords || !el.flowDot) return;
    if (!animate) {
      el.flowDot.style.transition = "none";
      el.flowDot.setAttribute("cx", coords.x);
      el.flowDot.setAttribute("cy", coords.y);
      el.flowDot.style.opacity = "1";
      void el.flowDot.getBoundingClientRect(); // 강제 리플로우 후 트랜지션 복원
      el.flowDot.style.transition = "";
    } else {
      el.flowDot.style.opacity = "1";
      el.flowDot.setAttribute("cx", coords.x);
      el.flowDot.setAttribute("cy", coords.y);
  ...
  function hideFlowDot() {
    if (el.flowDot) el.flowDot.style.opacity = "0";
  }
  ...
  /** 실제 기전: 단계마다 인체 도해가 해당 기관으로 이동·확대되며, 이동 표시점이 경로를 따라 움직입니다. */
  ...
  /* ------------------------------------------------------------------ */
  function updateStepFieldsVisibility() {
    const organ = $("#f-organ").value;
    el.stepFieldStomach.style.display = organ === "skin" || organ === "stomach" ? "none" : "";
    el.stepFieldIntestine.style.display = organ === "skin" || organ === "intestine" ? "none" : "";
    el.stepFieldLiver.style.display = organ === "skin" || organ === "liver" ? "none" : "";
    el.stepFieldKidney.style.display = organ === "skin" || organ === "kidney" ? "none" : "";
    el.stepFieldSkin.style.display = organ === "skin" ? "" : "none";
  }
  ...
  function openEditModal(id) {
    ...
    $("#f-real-simple").value = device.realMechanismSimple || "";
    $("#f-warn").value = device.sideEffects || "";

    const sc = device.stepCaptions || {};
    $("#f-step-stomach").value = sc.stomach || "";
    $("#f-step-intestine").value = sc.intestine || "";
    $("#f-step-liver").value = sc.liver || "";
    $("#f-step-kidney").value = sc.kidney || "";
    $("#f-step-skin").value = sc.skinEntry || "";
  ...
  function handleAddSubmit(ev) {
    ...
    const fields = {
      ...
      realMechanismSimple: $("#f-real-simple").value.trim(),
      sideEffects: $("#f-warn").value.trim(),
      stepCaptions: {
        stomach: $("#f-step-stomach").value.trim(),
        intestine: $("#f-step-intestine").value.trim(),
        liver: $("#f-step-liver").value.trim(),
        kidney: $("#f-step-kidney").value.trim(),
        skinEntry: $("#f-step-skin").value.trim(),
      },
    };
  ...
  function bindEvents() {
    ...
    el.fOrgan.addEventListener("change", updateStepFieldsVisibility);
    ...
    el.btnLevelSimple.addEventListener("click", () => setExplanationLevel("simple"));
    el.btnLevelDetailed.addEventListener("click", () => setExplanationLevel("detailed"));
